...
#include "../prefetcher/augur/augur.h"
...
void LSQ_ENTRY::finish(ooo_model_instr& rob_entry) const
{
  ...
  uint64_t addr = virtual_address.to<uint64_t>();

  // ===== STORE HOOK =====
  if (!rob_entry.destination_memory.empty()) {
      augur_set_value(addr, addr);
  }

  // ===== LOAD HOOK =====
  if (!rob_entry.source_memory.empty()) {

      rob_entry.offset = addr & 0x3F;
      rob_entry.base_addr = addr - rob_entry.offset;

      rob_entry.mem_value = augur_get_value(addr);
  }
}
